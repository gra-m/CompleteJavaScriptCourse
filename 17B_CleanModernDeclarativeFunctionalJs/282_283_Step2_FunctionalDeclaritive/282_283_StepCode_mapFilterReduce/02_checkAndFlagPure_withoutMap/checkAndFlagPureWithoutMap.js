...
//128 -> shows how the if statement could change entry of budget but ternary could not, as
// trying to add new object to frozen object.
// Instead a whole new budgetState array is made.
...
// for expenses not entered via this app.
const checkAndFlagExpenses = function (budgetState) {

    // before map have to create new array manually, ternary tries to replace entry of frozen
    // object, if statement just adds a property:
    const returnState = [];
    for (let entry of budgetState) {
        /*if (entry.value < -getLimit(personalSpendLimits, entry.user))
            entry.flag = 'limit-exceeded';*/
        entry = (entry.value < -getLimit(personalSpendLimits, entry.user)) ? returnState.push({...entry, flag:
         'limit-exceeded'}) : returnState.push(entry);
    }
    return returnState;
};

const returnState = checkAndFlagExpenses(budget);
console.log('check and flag', returnState);
