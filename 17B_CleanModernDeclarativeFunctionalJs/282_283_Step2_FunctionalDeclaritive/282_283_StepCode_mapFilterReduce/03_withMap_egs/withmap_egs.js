...
//Map Step 01 function expression returning new iterable where each entry is either flagged or
// returned:
/*
const checkAndFlagExpenses = function (budgetState) {
    return budgetState.map((entry, index, array)  => {
        return (entry.value < -getLimit(personalSpendLimits, entry.user)) ? {...entry, flag:
                'limit-exceeded'} : entry;
    });
};
*/

//Map Step 02 map call function has only the argument it needs (entry) and is then simplified:
/*const checkAndFlagExpenses = function (budgetState) {
    return budgetState.map(entry  => (entry.value < -getLimit(personalSpendLimits, entry.user)) ? {...entry, flag: 'limit-exceeded'} : entry);
};*/

//Map Step 03_withMap_egs Maps encapsulating function expression is itself changed to an arrow function:
