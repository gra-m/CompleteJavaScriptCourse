...
const calcDisplaySummary = function(account) {
    ...
    const interest = account.movements.filter(deposit =>
        deposit >  0)
...
btnTransfer.addEventListener('click', function (e) {
    ...
    const receiverAccount = accounts.find(function (acc) { return acc.userName === inputTransferTo.value});
...
btnLogin.addEventListener('click', function(e) {
    e.preventDefault();
    console.log('until html default behaviour for button in a form element is switched off, page' +
        ' will instantly refresh following event, so pass function an event param.');
